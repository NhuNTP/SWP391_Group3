...
public class DeleteAccountController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("id") == null || request.getParameter("id").equals("")) {
            ...
            request.getRequestDispatcher("ManageAccount/DeleteAccount.jsp").forward(request, response);
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("UserIdHidden") != null) {
            int id = Integer.parseInt(request.getParameter("UserIdHidden"));
            AccountDAO dao = new AccountDAO();
            int count = dao.Delete(id);
            if (count > 0) {
                response.sendRedirect("ViewAccountList");
            } else {
                response.sendRedirect("DeleteAccount?id=" + id);
            }
        }
    }
